...
class Pilha {
	...
	/*
	 * Realiza o empilhamanto
	 * @param Jogador jogador - jogador a ser impilhado
	 * */
	 ...
	 * Mostra todos os jogadores presentes na lista
	 * */
	int mostrar(Celula celula){
		Jogador player;
		int x = -1;
	
		if(celula != null && celula.elemento != null){
			x = mostrar(celula.prox) + 1;
			player = celula.elemento;
			MyIO.println("[" + x + "] ## " 
				         + player.getNome()  + " ## " +
					 player.getAltura()  + " ## " +
				           player.getPeso()  + " ## " + 
