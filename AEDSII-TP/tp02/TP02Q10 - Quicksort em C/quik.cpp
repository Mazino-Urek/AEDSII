...
/*
 * ordena um vetor de jogadores pelo estado de nascimento e caso empate pelo nome
...
void quickSort (Jogador *array, int esq, int dir){
	...
	while(i <= j){
		...
		nComparacao++;
		while(strcmp(array[i].estadoNascimento, pivo.estadoNascimento) <= 0 && aux){
			nComparacao += 2;       
			if(strcmp(array[i].estadoNascimento, pivo.estadoNascimento) != 0 ){
				i++;
			}
			else{
				nComparacao++;
				if(strcmp(array[i].nome , pivo.nome) < 0){
					i++;
				}
				else
					aux = false;
		...
		aux = true;
		nComparacao++;
		while(strcmp(array[j].estadoNascimento, pivo.estadoNascimento) >= 0 && aux){
			nComparacao += 2;
		       	if(strcmp(array[j].estadoNascimento, pivo.estadoNascimento) != 0 ){
				j--;
			}
			else{
				nComparacao++;
				if(strcmp(array[j].nome , pivo.nome) > 0){
					j--;
				}
				else
					aux = false;
...
int main (void){
	...
	FILE *arq = fopen("710678_quicksort.txt", "w");
